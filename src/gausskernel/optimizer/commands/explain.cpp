...
{    
    ...
    if(query_info->dop == 1){
        query_info->query_used_mem = (int64)(t_thrd.utils_cxt.peakedBytesInQueryLifeCycle / 1024);
    } else {
        ...
        //     if (false == entry->isValid || 0 == (entry->peakChunksQuery - entry->initMemInChunks))
        //         continue;
        //     query_info->query_used_mem += (entry->peakChunksQuery - entry->initMemInChunks) * 1024;
        // }
...
void FixExecutionTimes(knl_query_info_context* query_info){
    for (auto& plan_pair: query_info->Plans){
        plan_pair.second.execution_time = plan_pair.second.total_time - plan_pair.second.start_up_time;
    }
}
...
static void get_datanode_info(knl_plan_info_context &plan_info, PlanState *planstate)
{
    ...
    double start_up_sec_min = HUGE_VAL - 1;
    double rows = 0;
    double exec_sec;
    double start_up_sec;
            ...
            for (i = 0; i < u_sess->instr_cxt.global_instr->getInstruNodeNum(); i++) {
                ...
                for (j = 0; j < dop; j++) {
                    ...
                    if (instr != NULL && instr->nloops > 0) {
                        ...
                        start_up_sec_min = rtl::min(start_up_sec_min, start_up_sec);
