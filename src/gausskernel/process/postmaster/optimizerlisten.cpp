...
#include <vector>
std::vector<uint64_t> epoch_listen_thread_ids;
...
void OptimizerListenMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext optimizerlisten_context;
     ...
     */
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "optimizer listen", THREAD_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_STORAGE));
     ...
     */
    optimizerlisten_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "optimizer listen",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(optimizerlisten_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
         ...
         * about in epochlisten, but we do have LWLocks, and perhaps buffers?
         */
        LWLockReleaseAll();
        ...
        MemoryContextResetAndDeleteChildren(optimizerlisten_context);
    ...
    pgstat_report_appname("optimizer listen");
    ...
    for (;;) {
         ...
         */
        // if (t_thrd.epochlisten_cxt.got_SIGHUP) {
        //     t_thrd.epochlisten_cxt.got_SIGHUP = false;
        //     ProcessConfigFile(PGC_SIGHUP);
        // }

        // if (t_thrd.epochlisten_cxt.shutdown_requested) {
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.optimizerlisten_cxt.got_SIGHUP = true;
...
static void _ShutdownHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.optimizerlisten_cxt.shutdown_requested = true;
