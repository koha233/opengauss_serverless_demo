                               ...
                               const char *paramModes, int numParams) /* number of parameters */
{
    ...
    CompleteCachedPlan(psrc, querytree_list, unnamed_stmt_context, paramTypes, paramModes, numParams, NULL, NULL,
                       ...
                       stmt_name, single_exec_node, is_read_only);
        ...
        switch (firstchar) {
            ...
            case 'o': {
                ...
                if (unlikely(IsAbortedTransactionBlockState())) {
                    if (InReceivingLocalUserIdChange()) {
                        SetUserIdAndSecContext(GetOldUserId(true), u_sess->misc_cxt.SecurityRestrictionContext &
                                                                       (~RECEIVER_LOCAL_USERID_CHANGE));
                    }
                    break;
                }
            ...
            case 'r': /* query id with sync */
            {
                ...
                /* Set the query id we were passed down */
                errno_t rc = memcpy_s(&u_sess->debug_query_id, sizeof(uint64),
                                      pq_getmsgbytes(&input_message, sizeof(uint64)), sizeof(uint64));
                securec_check(rc, "\0", "\0");
                ereport(DEBUG1, (errmsg("Received new query id %lu", u_sess->debug_query_id)));
                rc = memcpy_s(&oidcount, sizeof(int), pq_getmsgbytes(&input_message, sizeof(int)), sizeof(int));
                securec_check(rc, "\0", "\0");
                for (int i = 0; i < oidcount; i++) {
                    Oid tmp = InvalidOid;
                    rc = memcpy_s(&tmp, sizeof(Oid), pq_getmsgbytes(&input_message, sizeof(Oid)), sizeof(Oid));
                    securec_check(rc, "\0", "\0");
            ...
            case 's': /* snapshot */
            {
                ...
                int satisfies;
                rc = memcpy_s(&satisfies, sizeof(int), pq_getmsgbytes(&input_message, sizeof(int)), sizeof(int));
                securec_check(rc, "\0", "\0");
